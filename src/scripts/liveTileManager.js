...
function generateRandomAccent() {
    /*const accentColor = DiscoBoard.backendMethods.serveConfig().accentcolor;
    return adjustColor(accentColor, Math.floor(Math.random() * 100) - 50);*/
    return "var(--accent-color-shade-" + Math.floor(Math.random() * 3) + ")";
}
...
function uninitializeLiveTile(packageName) {
    const tiles = document.querySelectorAll(`div.disco-home-tile.live-tile[packagename="${packageName}"]`);
    tiles.forEach(tile => {
