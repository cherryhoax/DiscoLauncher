...
window.discoThemes = discoThemes;
...
function alert(title = "Alert!", message = "Message", actions = [{ title: "" }], inline) {
  ...
  actions.forEach(element => {
    ...
    items += `<button class="disco-element disco-element-alert-action${element["style"] == "default" ? " default" : element["style"] == "destructive" ? " destructive" : ""}">
        ${element.title}
      </button>`

  });
  $("body").append(`
    <div class="disco-element disco-element-alert">
      <p class="disco-element-alert-title">${title}</p>
      <p class="disco-element-alert-body">${message}</p>
      ${items}
  </div>`)
  if ($("div.disco-element-alert-layer:not(.taken)").length == 0) {
    $("body").append(`
    <div class="disco-element disco-element-alert-layer">
     
  </div>`)
  }
  var alertmenu = $("div.disco-element.disco-element-alert").last()
  if ((actions.length <= 2 && !inline)) alertmenu.addClass("inline")
  actions.forEach((element, index) => {
    if (element["action"]) {
      if (typeof element["action"] == "function") {
        alertmenu.children("button.disco-element-alert-action").eq(index).on("click", function () {
          setTimeout(() => {
  ...
  function closeW() {
    if ($("div.disco-element.disco-element-alert").length == 1) {
      var layer = $("div.disco-element-alert-layer").addClass("taken")
      layer.addClass("exit")
      setTimeout(() => {
        layer.remove()
      }, 200);
  ...
  alertmenu.on("click", function (e) {
    ...
    if (e.target.classList.contains("disco-element-alert-action")) {
      closeW();
    }
  });
...
const boardMethods = {
  ...
  createHomeTile: (size = [1, 1], options = {}, append = false) => {
    ...
    const el = DiscoElements.wHomeTile(
      options.imageIcon,
      options.icon,
      options.title,
      options.packageName,
  ...
  createAppTile: (options) => {
    ...
    const el = DiscoElements.wAppTile(
      options.icon,
      options.iconbg,
      options.title,
      options.packageName
  ...
  createLetterTile: (letter) => {
    const el = DiscoElements.wLetterTile(letter);
    document
      .querySelector(
        "#main-home-slider > div > div:nth-child(2) > div > div.app-list > div.app-list-container"
      )
  ...
  createAppMenu: (packageName) => {
    ...
    entries[window.i18n.t("common.app_menu.pin_to_home")] = () => {
      const findTile = $(
        `div.inner-page.app-list-page > div.app-list > div.app-list-container > div.disco-element.disco-app-tile[packagename="${packageName}"]`
      )[0];
      const iconpack = findTile.classList.contains("iconpack0")
        ...
        : findTile.classList.contains("iconpack1")
          ? 1
          : 2;
      const el = DiscoBoard.boardMethods.createHomeTile(
        [2, 2],
        {
    ...
    entries[window.i18n.t("common.app_menu.app_info")] = () => {
      Disco.launchAppInfo(packageName)
    }
    entries[window.i18n.t("common.app_menu.uninstall")] = () => {
      if (DiscoBoard.backendMethods.packageManagerProvider.get() == 0) {
        Disco.uninstallApp(packageName, 0);
      } else {
        parent.DiscoBoard.alert(
          ...
          [{
            title: window.i18n.t("common.actions.yes"), style: "default", inline: true, action: () => {
              Disco.uninstallApp(packageName, DiscoBoard.backendMethods.packageManagerProvider.get());
            }
          }, { title: window.i18n.t("common.actions.no"), style: "default", inline: true, action: () => { } }]
        );
      }
    }
    const el = DiscoElements.wAppMenu(packageName, entries);
    el.fullHeight = 219
    el.elementHeight = 65
    ...
    if (document.querySelectorAll(`div.disco-home-tile[packagename="${packageName}"]`).length > 0) {
  ...
  createTileMenu: (el) => {
    ...
    const tileMenu = DiscoElements.wTileMenu(el);
    el.appendChild(tileMenu);
    return el;
  },
  createAppView: (packageName, args) => {
    const appView = DiscoElements.wAppView(packageName, args);
...
const backendMethods = {
  ...
  reloadApps: function (callback) {
    ...
    var appsWithPreferences = JSON.parse(JSON.stringify(backendMethods.reloadAppDatabase())).map(e => {
      const appPreference = DiscoBoard.backendMethods.getAppPreferences(e.packageName);
  ...
  getAppDetails: (packageName, rawDetails = false) => {
    ...
    if (Object.keys(iconPackDB).includes(packageName)) {
      ...
      if (idb["accent"]) {
        ...
        if (discoColors[idb.accent]) color = discoColors[idb.accent]; else color = idb.accent;
        ...
        //el.querySelector("img.disco-app-tile-imageicon").style.backgroundColor = `var(--metro-color-${idb.accent})`
  ...
  reloadAppDatabase: () => {
    var apps = JSON.parse(Disco.retrieveApps());
  ...
  refreshInsets: () => {
    ...
    window.windowInsetsRaw = JSON.parse(Disco.getSystemInsets());
  ...
  navigation: {
    ...
    push: (change, forwardAction, backAction, homeBack = true) => {
      DiscoBoard.backendMethods.navigation.invalidate(change);
      forwardAction();
    ...
    get lastPush() {
      if (DiscoBoard.backendMethods.navigation.history.length == 0)
        return undefined;
      return DiscoBoard.backendMethods.navigation.history.slice(-1)[0];
    },
    invalidate: (change) => {
      if (DiscoBoard.backendMethods.navigation.history.length == 0)
        return undefined;
  ...
  closeInternalApp: (packageName, homeBack = false) => {
    //Alert app
    try {
      appViewEvents.softExit(document.querySelector(`iframe.disco-element.disco-app-view[packageName="${packageName}"]`), homeBack)
      setTimeout(() => {
  ...
  setAccentColor: (color, doNotSave = false) => {
    ...
    document.querySelectorAll("iframe.disco-app-view").forEach(e => appViewEvents.setAccentColor(e, color))
    if (!doNotSave) localStorage.setItem("accentColor", color)
    if (window.Disco) Disco.setAccentColor(color)
    if (Object.values(discoColors).includes(color)) {
      //Disco.setAppIconColor(Object.entries(discoColors).filter(e => e[1].toLowerCase() == color.toLowerCase())[0][0])
    } else {
      console.error("Custom color detected!");
      //Disco.setAppIconColor("default")
    }
  },
  setTheme: (theme, doNotSave = false) => {
    if (Object.values(discoThemes).includes(theme)) {
      var applyTheme
      ...
      if (window.Disco) Disco.setNavigationBarAppearance(applyTheme ? "dark" : "light");
      if (window.Disco) Disco.setStatusBarAppearance(applyTheme ? "dark" : "light");
      document.querySelectorAll("iframe.disco-app-view").forEach(e => appViewEvents.setTheme(e, applyTheme))
      if (!doNotSave) {
  ...
  setTileColumns: (int, doNotSave = false) => {
    if (Object.values(discoTileColumns).includes(int)) {
  ...
  homeConfiguration: {
    ...
    load: () => {
      ...
      config.forEach(tile => {
        ...
        const homeTile = DiscoElements.wHomeTile(appdetail.icon.foreground, appdetail.icon.background, appdetail.label, tile.p, "", tile.s)
  ...
  wallpaper: {
    ...
    recalculateOffsets: (scrollpos) => {
      ...
      if (tileListInnerContainer.classList.contains("wallpaper-behind")) {
        ...
        $("div.tile-list-inner-container > div.disco-home-tile").each(
          (index, el) => {
  ...
  font: {
    ...
    set: async (font, doNotSave = false) => {
      ...
      fontStore.loadFont()
      document.querySelectorAll("iframe.disco-app-view").forEach(e => appViewEvents.setFont(e, font))
    },
    remove: () => {
      fontStore.clearFont()
    }
  ...
  appUninstall: (packagename) => {
    const tileElem = document.querySelector(`div.disco-home-tile[packagename="${packagename}"]`)
    if (tileElem) {
  ...
  setupNeeded: () => {
    ...
    else if (localStorage["lastVersion"] != Disco.getAppVersion()) return true;
    else return false;
  },
  ...
  animationDurationScale: {
    set: (scale) => {
      ...
      document.querySelectorAll("iframe.disco-app-view").forEach(e => appViewEvents.setAnimationDurationScale(e, scale))
    },
    get: () => {
  ...
  getAppTilePreferences: (packageName) => {
    ...
    if (window.Disco && window.Disco.getAppTilePreferences) {
      try {
        const prefsStr = window.Disco.getAppTilePreferences(packageName);
    ...
    // Check new perAppTilePreferences format (used by Disco Settings)
    ...
    const key = `disco_app_tiles_${packageName}`;
  ...
  setAppTilePreferences: (packageName, preferences) => {
    // Try to use WebInterface method first
    if (window.Disco && window.Disco.setAppTilePreferences) {
      try {
        window.Disco.setAppTilePreferences(packageName, JSON.stringify(preferences));
      } catch (error) {
  ...
  refreshAllTiles: () => {
    ...
    setTimeout(() => {
      document.querySelectorAll('.disco-home-tile[packagename]').forEach(tile => {
    ...
    setTimeout(() => {
      document.querySelectorAll('.disco-app-tile[packagename]').forEach(tile => {
        const packageName = tile.getAttribute('packagename');
        if (packageName) {
          backendMethods.applyTilePreferences(tile, packageName);
        }
  ...
  applyTilePreferences: (tileElement, packageName) => {
    ...
    const isHomeTile = tileElement.classList.contains('disco-home-tile');
    const isAppTile = tileElement.classList.contains('disco-app-tile');
    ...
    if (prefs.icon === 'monochrome' && iconElement) {
      ...
      if (window.Disco && window.Disco.supportsMonochromeIcons && window.Disco.supportsMonochromeIcons() === "true") {
...
window.addEventListener("load", () => {
  backendMethods.animationDurationScale.set(window["Disco"] ? Disco.getAnimationDurationScale() : 1)
