...
const CONSTANTS = {
    INTERNAL_SETTINGS_APP: 'disco.internal.settings',
    INTERNAL_STORE_APP: 'disco.internal.store',
    INTERNAL_TWEAKS_APP: 'disco.internal.tweaks',
...
class DiscoMock {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                this.#retrievedApps.push({
                    ...
                    label: 'Disco Settings',
                    type: 0
                });
                this.#retrievedApps.push({
                    packageName: CONSTANTS.INTERNAL_STORE_APP,
                    label: 'Disco Web Store',
                    type: 0
                });
                this.#retrievedApps.push({
    ...
    launchApp(packageName) {
        ...
        if (packageName.startsWith("disco.internal")) {
            if (packageName.includes("?")) {
                DiscoBoard.backendMethods.launchInternalApp(packageName.split("?")[0], packageName.split("?")[1]);
            } else {
                DiscoBoard.backendMethods.launchInternalApp(packageName);
            }
        }
        return true
    }
    ...
    getAPILevel() {
        ...
        const configuredLevel = localStorage.getItem("disco_mock_api_level");
    ...
    setMonochromeIcons(enable) {
        localStorage.setItem("disco_monochrome_icons", enable.toString());
    ...
    getMonochromeIcons() {
        //testing
        return false
        return localStorage.getItem("disco_monochrome_icons") === "true" ? "true" : "false";
    }
    ...
    getAppTilePreferences(packageName) {
        ...
        // Check new perAppTilePreferences format first (used by Disco Settings)
