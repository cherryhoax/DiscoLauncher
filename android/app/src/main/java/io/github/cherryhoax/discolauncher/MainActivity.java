...
public class MainActivity extends AppCompatActivity {
    ...
    public DiscoWebView webView;
    public DiscoGeckoView discoView; // <-- keep this, but only use it if GeckoView is present
    ...
    protected void onResume() {
        if (pauseRunnable != null) {
            ...
            Log.d("discolauncher", "homeButtonPress: ");
        } else {
            if (activityDispatchEvent) {
                if (webEngine.equals("WebView"))
                    webEvents.dispatchEvent(WebEvents.events.activityResume, null);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (webEngine.equals("WebView")) {
            ...
            discoView.setLayoutParams(new ConstraintLayout.LayoutParams(
                    ...
                    ConstraintLayout.LayoutParams.MATCH_PARENT));
            ConstraintLayout mainLayout = findViewById(R.id.main);
            mainLayout.addView((android.view.View) discoView);
        }
...
// Add a listener for configuration changes to detect system theme changes
    ...
    public boolean onCreateThumbnail(Bitmap outBitmap, Canvas canvas) {
        String response = "1";
        try {
            response = new JSONObject(webView.evaluateJavascriptSync("DiscoBoard.backendMethods.serveConfig()")).optString("theme", "1");
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    ...
    private void handleDiscoUrl(String url) throws JSONException {
