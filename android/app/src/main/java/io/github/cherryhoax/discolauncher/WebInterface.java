...
public class WebInterface {
    ...
    public String retrieveApps() throws JSONException {
        ...
        discoSettings.put("type", 0);
        retrievedApps.put(discoSettings);
        JSONObject discoTweaks = new JSONObject();
        discoTweaks.put("packageName", "disco.internal.tweaks");
        discoTweaks.put("label", "Disco Tweaks");
        discoTweaks.put("type", 0);
        retrievedApps.put(discoTweaks);
        ...
        if (packageName.startsWith("disco.internal")) {
            if (packageName.contains("?")) {
                mainActivity.webView.post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        mainActivity.webView.evaluateJavascript(
                                "window.DiscoBoard.backendMethods.launchInternalApp(\"" + parts[0] + "\",\"" + parts[1] + "\")",
                                null);
                    }
                });
            } else {
                mainActivity.webView.post(new Runnable() {
                    ...
                    public void run() {
                        mainActivity.webView.evaluateJavascript(
                                "window.DiscoBoard.backendMethods.launchInternalApp(\"" + packageName + "\")", null);
                    }
                });
            }
    ...
    public boolean uninstallApp(String packageNameWithIntent, int packageManagerProvider) {
        ...
        Log.d("discolaauncher", "uninstallApp pm provider: " + packageManagerProvider);
    ...
    public String getContacts() throws JSONException {
        return DiscoExperience.getContacts(mainActivity);
    }

    @JavascriptInterface
    public String getPhotos() throws JSONException {
        return DiscoExperience.getPhotos(mainActivity);
    }
    ...
    public void requestPermission(String permission) {
        mainActivity.runOnUiThread(() -> {
            if ("CONTACTS".equals(permission)) {
                Log.d("discolauncher", "checkPermission: " + permission);
    ...
    public void applyIconPackPerApp(String appPackageName, String iconPackPackageName) {
        ...
        Log.d("DiscoLauncher", "Applied icon pack " + iconPackPackageName + " for app " + appPackageName);
    }
    ...
    public void setMonochromeIcons(boolean enable) {
        ...
        SharedPreferences prefs = mainActivity.getSharedPreferences("disco_settings", Context.MODE_PRIVATE);
        prefs.edit().putBoolean("monochrome_icons", enable).apply();
        
        Log.d("DiscoLauncher", "Monochrome icons setting: " + enable);
    ...
    public void setAppTilePreferences(String packageName, String preferences) {
        SharedPreferences prefs = mainActivity.getSharedPreferences("disco_app_tiles", Context.MODE_PRIVATE);
        prefs.edit().putString(packageName, preferences).apply();
        Log.d("DiscoLauncher", "Set tile preferences for " + packageName + ": " + preferences);
    }
    ...
    public void removeAppTilePreferences(String packageName) {
        ...
        Log.d("DiscoLauncher", "Removed tile preferences for " + packageName);
    }
}